...
public:
    ...
    // Constructor Swerve
    Swerve(double vx, double vy, double omega){
        this->vx = vx;
        this->vy = vy;
        this->omega = omega;
...
//      vnxy.display();
        ...
        for (int i = 0; i < 4; i++){
            vector<double> b;
            for (int j = 0; j < 1; j++){
                double vn = sqrt(vnxy.element[2*i][0]*vnxy.element[2*i][0] + vnxy.element[2*i+1][0]*vnxy.element[2*i+1][0]);
                b.push_back(vn);}
            a.push_back(b);}
        v.element = a;
        // tes print
    ...
    Matrix velocityCommand(double vx, double vy, double omega)
    {
    ...
    Matrix updatePose(Matrix Pose, double dt, int it){
...
int main(){
    ...
    while (t <= 630){
        double ti = t/100.f;
        double vxt = 48 * cos(ti) * (sin(ti) * sin(ti));
        double vyt = 4 * sin(4*ti) + 6 * sin(3*ti) + 10 * sin(2*ti) - 13 * sin(ti);
        double omega = 0;
